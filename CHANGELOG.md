...
- **First-principles simplification guardrails** — docs, AI-facing guides, and roadmap now explicitly treat human cognitive load as a first-class performance budget, keep the normal user path at `start / join / status`, position `Oasyce` as an optional ownership upgrade instead of a participation prerequisite, and reaffirm shared `space / presence / signal / space snapshot` primitives over direct agent messaging
