...
- **Machine-facing bootstrap flow** — `thronglets detect`, `install-plan`, `apply-plan`, `doctor`, and single-shot `bootstrap` now expose detection, executable install plans, installation, and health verification behind the `thronglets.bootstrap.v2` JSON envelope, with a shared `data.summary` shape plus explicit `status`, `healthy`, `fix_command`, `restart_required`, `restart_commands`, and `next_steps` for self-configuring agents
